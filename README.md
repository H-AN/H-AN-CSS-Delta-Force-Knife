...
- **三套音效系统**：
  ...
  - 转刀音支持**逗号分隔多条组合音效**（如 `"5:xx/a.wav,33:xx/b.wav"`），每条按各自帧号独立延迟播放，共用转刀通道，后者自然打断前者
  - **命中音**（仅攻击者可听）
  - **击杀音**：根据最近命中部位自动区分**爆头击杀音 / 普通击杀音**
  - 任何一个音效路径**留空即不播放**
- **动画驱动**：通过 HanWeaponSystem 双 ViewModel 自定义动画接口播放（VM0 序列清零 + 1 号 VM 播放自定义序列）
- **双切换动画**（可选）：每次掏刀 50% 走原版同步切换、50% 播放配置的特殊切换序列，两个切换各配独立音效；攻击优先，出刀会正常打断切换动画
...
| `RotateSound1~3` | 左键各刀转刀音效，格式 **`帧号:路径`** = 动画第 N 帧开始播放（不带帧号 = 第 0 帧立即播放；留空 = 不播放）。支持**逗号分隔多条组合音效**（如 `"5:xx/a.wav,33:xx/b.wav"` = 第 5 帧播 a、第 33 帧播 b），每条独立延迟，共用通道后者打断前者，最多 4 条 | 空 |
| `RightRotateSound` | 右键转刀音效，格式同 `RotateSound1~3`，同样支持逗号分隔多条 | 空 |
| `DrawSequence2` | 特殊切换的 QC 序列号。**填了才启用双切换动画**：每次掏刀 50% 走原版同步切换、50% 播放此序列；不填 = 不干预掏刀 | 空 |
| `DrawFrames2` | 特殊切换动画总帧数（状态时长 = 帧数 ÷ 30 秒，必须覆盖整个切换动画） | 无 |
| `DrawSound1` | 普通切换音效（走同步 draw 时播放；留空 = 不播放） | 空 |
| `DrawSound2` | 特殊切换音效（走特殊 draw 时播放；留空 = 不播放） | 空 |
...
"my_new_knife"
{
    ...
    "RotateSound3"      "7:weapons/mynewknife/fire.wav,43:weapons/mynewknife/rotate_3.wav"   // 拼接多音效: 第 7 帧播 fire, 第 43 帧播 rotate_3
...
- 启用双切换动画时，该武器在**武器系统配置内的 `switchsound` 必须留空**，否则每次掏刀会叠加播放固定切换音效
- **普通切换自带原版切换音**（内嵌在原 draw 动画的音效事件里，经音效替换钩子处理），因此 `DrawSound1` 通常留空；特殊切换动画没有内嵌音效，由 `DrawSound2` 提供
- 击杀音判定使用"最近一次命中部位"；致命一击不播命中音（转而播击杀音）
- **任何武器切换都会重置连招**（切走/切回/拾取），再次掏出三角洲刀必定从第一刀开始；切换同时作废尚未播放的延迟转刀音
